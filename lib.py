...
class Operation:
    ...
    positions = ["lineno", "endlineno", "col_offset", "end_col_offset"]
    opcode_names = [
        'POP_TOP', 'ROT_TWO', 'ROT_THREE', 'DUP_TOP', 'DUP_TOP_TWO', 'ROT_FOUR', 'NOP', 'UNARY_POSITIVE', 'UNARY_NEGATIVE', 'UNARY_NOT', 'UNARY_INVERT', 'BINARY_MATRIX_MULTIPLY', 'INPLACE_MATRIX_MULTIPLY', 'BINARY_POWER', 'BINARY_MULTIPLY', 'BINARY_MODULO', 'BINARY_ADD', 'BINARY_SUBTRACT', 'BINARY_SUBSCR', 'BINARY_FLOOR_DIVIDE', 'BINARY_TRUE_DIVIDE', 'INPLACE_FLOOR_DIVIDE', 'INPLACE_TRUE_DIVIDE', 'GET_LEN', 'MATCH_MAPPING', 'MATCH_SEQUENCE', 'MATCH_KEYS', 'COPY_DICT_WITHOUT_KEYS', 'WITH_EXCEPT_START', 'GET_AITER', 'GET_ANEXT', 'BEFORE_ASYNC_WITH', 'END_ASYNC_FOR', 'INPLACE_ADD', 'INPLACE_SUBTRACT', 'INPLACE_MULTIPLY', 'INPLACE_MODULO', 'STORE_SUBSCR', 'DELETE_SUBSCR', 'BINARY_LSHIFT', 'BINARY_RSHIFT', 'BINARY_AND', 'BINARY_XOR', 'BINARY_OR', 'INPLACE_POWER', 'GET_ITER', 'GET_YIELD_FROM_ITER', 'PRINT_EXPR', 'LOAD_BUILD_CLASS', 'YIELD_FROM', 'GET_AWAITABLE', 'LOAD_ASSERTION_ERROR', 'INPLACE_LSHIFT', 'INPLACE_RSHIFT', 'INPLACE_AND', 'INPLACE_XOR', 'INPLACE_OR', 'LIST_TO_TUPLE', 'RETURN_VALUE', 'IMPORT_STAR', 'SETUP_ANNOTATIONS', 'YIELD_VALUE', 'POP_BLOCK', 'POP_EXCEPT', 'UNPACK_SEQUENCE', 'UNPACK_EX', 'ROT_N', 'LOAD_CONST', 'BUILD_TUPLE', 'BUILD_LIST', 'BUILD_SET', 'BUILD_MAP', 'COMPARE_OP', 'IS_OP', 'CONTAINS_OP', 'RERAISE', 'LOAD_FAST', 'STORE_FAST', 'DELETE_FAST', 'GEN_START', 'RAISE_VARARGS', 'CALL_FUNCTION', 'MAKE_FUNCTION', 'BUILD_SLICE', 'LOAD_CLOSURE', 'LOAD_DEREF', 'STORE_DEREF', 'DELETE_DEREF', 'CALL_FUNCTION_KW', 'CALL_FUNCTION_EX', 'EXTENDED_ARG', 'LIST_APPEND', 'SET_ADD', 'MAP_ADD', 'LOAD_CLASSDEREF', 'MATCH_CLASS', 'FORMAT_VALUE', 'BUILD_CONST_KEY_MAP', 'BUILD_STRING', 'CALL_METHOD', 'LIST_EXTEND', 'SET_UPDATE', 'DICT_MERGE', 'DICT_UPDATE' 
    ]
    opcode_numbers =  list(range(1, len(opcode_names)+1))
    code_map = {
        ...
        "COPY":"Push the i-th item to the top of the stack. The item is not removed from its original location.",
        "SWAP":"Swap TOS with the item at position i.",
        ...
        "SET_ADD":"Calls set.add(TOS1[-i], TOS). Used to implement set comprehensions.",
        "LIST_APPEND":"Calls list.append(TOS1[-i], TOS). Used to implement list comprehensions.",
        "MAP_ADD":"Calls dict.__setitem__(TOS1[-i], TOS1, TOS). Used to implement dict comprehensions.",
        ...
        'CHECK_EG_MATCH': 'Performs exception matching for except*. Applies split(TOS) on the exception group representing TOS1.', 
        'PREP_RERAISE_STAR': 'Combines the raised and reraised exceptions list from TOS, into an exception group to propagate from a try-except* block. Uses the original exception group from TOS1 to reconstruct the structure of reraised exceptions. Pops two items from the stack and pushes the exception to reraise or None if there isnt one.', 
        ...
        "LOAD_CONST": "Pushes a constant to the stack.",
        "IMPORT_NAME": "Imports the module co_names[namei]. TOS and TOS1 are popped and provide the fromlist and level arguments of __import__(). The module object is pushed onto the stack. The current namespace is not affected: for a proper import statement, a subsequent STORE_FAST instruction modifies the namespace.",
        "STORE_NAME": "Implements name = TOS. namei is the index of name in the attribute co_names of the code object. The compiler tries to use STORE_FAST or STORE_GLOBAL if possible.",
        "LOAD_NAME": "Pushes the value associated with co_names[namei] onto the stack.",
        "LOAD_ATTR": "Replaces TOS with getattr(TOS, co_names[namei]).",
        ...
        "CALL_FUNCTION_KW": "Calls a callable object with keyword arguments, if the lowest bit of flags is set, the top of the stack contains a mapping object containting additonal keyword arguments. Before the callable is called, the mapping object and iterable object are each “unpacked” and their contents passed in as keyword and positional arguments respectively. CALL_FUNCTION_EX pops all arguments and the callable object off the stack, calls the callable object with those arguments, and pushes the return value returned by the callable object.",
        "MAKE_FUNCTION": """Pushes a new function object on the stack. From bottom to top, the consumed stack must consist of values if the argument carries a specified flag value
        ...
        'ROT_THREE': 'Lifts second and third stack items one position up, moves top down to position three',
        'DUP_TOP': 'Duplicates the reference on top of the stack',
        'DUP_TOP_TWO': 'Duplicates the two references on top of the stack',
        'NOP': 'Does nothing',
        'UNARY_POSITIVE': 'Implements positive unary operator (+x)',
        'UNARY_NEGATIVE': 'Implements negative unary operator (-x)',
        'UNARY_NOT': 'Implements logical NOT operator (not x)',
        'UNARY_INVERT': 'Implements bitwise NOT operator (~x)',
        'BINARY_MATRIX_MULTIPLY': 'Implements matrix multiplication operator (@)',
        'INPLACE_MATRIX_MULTIPLY': 'Implements in-place matrix multiplication operator (@=)',
        'BINARY_POWER': 'Implements exponentiation operator (x ** y)',
        'BINARY_MULTIPLY': 'Implements multiplication operator (x * y)',
        'BINARY_MODULO': 'Implements modulo operator (x % y)',
        'BINARY_ADD': 'Implements addition operator (x + y)',
        'BINARY_SUBTRACT': 'Implements subtraction operator (x - y)',
        'BINARY_SUBSCR': 'Implements subscription operator (x[y])',
        'BINARY_FLOOR_DIVIDE': 'Implements floor division operator (x // y)',
        'BINARY_TRUE_DIVIDE': 'Implements true division operator (x / y)',
        'INPLACE_FLOOR_DIVIDE': 'Implements in-place floor division operator (x //= y)',
        'INPLACE_TRUE_DIVIDE': 'Implements in-place true division operator (x /= y)',
        'GET_AITER': 'Loads the async iterator for an asynchronous iterable',
        'GET_ANEXT': 'Loads the next value from an asynchronous iterator',
        'BEFORE_ASYNC_WITH': 'Sets up a coroutine for an asynchronous with statement',
        'BEGIN_FINALLY': 'Starts the "finally" block of a try statement',
        'END_ASYNC_FOR': 'Ends the loop of an asynchronous for statement',
        'INPLACE_ADD': 'Implements in-place addition operator (x += y)',
        'INPLACE_SUBTRACT': 'Implements in-place subtraction operator (x -= y)',
        'INPLACE_MULTIPLY': 'Implements in-place multiplication operator (x *= y)',
        'INPLACE_MODULO': 'Implements in-place modulo operator (x %= y)',
        'STORE_SUBSCR': 'Implements subscription assignment (x[y] = z)',
        'DELETE_SUBSCR': 'Implements deletion of subscripted item (del x[y])',
        'BINARY_LSHIFT': 'Implements bitwise left shift operator (x << y)',
        'BINARY_RSHIFT': 'Implements bitwise right shift operator (x >> y)',
        'BINARY_AND': 'Implements bitwise AND operator (x & y)',
        'BINARY_XOR': 'Implements bitwise XOR operator (x ^ y)',
        'BINARY_OR': 'Implements bitwise OR operator (x | y)',
        'INPLACE_POWER': 'Implements in-place exponentiation operator (x **= y)',
        'GET_ITER': 'Gets an iterator from an object',
        'GET_YIELD_FROM_ITER': 'Gets the value to yield from an iterator',
        'PRINT_EXPR': 'Prints the value of an expression',
        'LOAD_BUILD_CLASS': 'Loads the __build_class__ function',
        'YIELD_FROM': 'Resumes the execution of a generator function or a subiterator',
        'GET_AWAITABLE': 'Gets the awaitable object to be awaited',
        'INPLACE_LSHIFT': 'Implements in-place bitwise left shift operator (x <<= y)',
        'INPLACE_RSHIFT': 'Implements in-place bitwise right shift operator (x >>= y)',
        'INPLACE_AND': 'Implements in-place bitwise AND operator (x &= y)',
        'INPLACE_XOR': 'Implements in-place bitwise XOR operator (x ^= y)',
        'INPLACE_OR': 'Implements in-place bitwise OR operator (x |= y)',
        'BREAK_LOOP': 'Terminates a loop or a switch statement',
        'WITH_CLEANUP_START': 'Starts the clean-up of a with statement',
        'WITH_CLEANUP_FINISH': 'Finishes the clean-up of a with statement',
        'RETURN_VALUE': 'Returns with the TOS as the result',
        'IMPORT_STAR': 'Imports all names from a module',
        'SETUP_ANNOTATIONS': 'Sets up annotations for a function or a class',
        'YIELD_VALUE': 'Yields a value from a generator function',
        'POP_BLOCK': 'Removes one block from the block stack',
        'END_FINALLY': 'Ends the "finally" block of a try statement',
        'POP_EXCEPT': 'Ends a "try" block with an exception',
        'STORE_NAME': 'Stores TOS as a local or global name',
        'DELETE_NAME': 'Deletes the name specified in the TOS',
        'UNPACK_SEQUENCE': 'Unpacks an iterable into individual values',
        'FOR_ITER': 'Creates an iterator and pushes it on the stack',
        'UNPACK_EX': 'Unpacks an iterable into individual values with extended unpacking',
        'STORE_ATTR': 'Stores the TOS object attribute named co_names[namei]',
        'DELETE_ATTR': 'Deletes the TOS object attribute named co_names[namei]',
        'STORE_GLOBAL': 'Stores TOS in the global name space',
        'DELETE_GLOBAL': 'Deletes a name from the global name space',
        'LOAD_CONST': 'Pushes a constant value onto the stack',
        'LOAD_NAME': 'Pushes the value associated with co_names[namei] onto the stack',
        'BUILD_TUPLE': 'Builds a tuple from the topmost items on the stack',
        'BUILD_LIST': 'Builds a list from the topmost items on the stack',
        'BUILD_SET': 'Builds a set from the topmost items on the stack',
        'BUILD_MAP': 'Builds a dictionary from the topmost items on the stack',
        'LOAD_ATTR': 'Pushes the value of the named attribute onto the stack',
        'COMPARE_OP': 'Performs a comparison between two values',
        'IMPORT_NAME': 'Imports the module co_names[namei]',
        'IMPORT_FROM': 'Loads the attribute co_names[namei] from the module',
        'JUMP_FORWARD': 'Increments the bytecode counter by delta',
        'JUMP_IF_FALSE_OR_POP': 'If TOS is false, sets the bytecode counter to target',
        'JUMP_IF_TRUE_OR_POP': 'If TOS is true, sets the bytecode counter to target',
        'JUMP_ABSOLUTE': 'Sets the bytecode counter to target',
        'POP_JUMP_IF_FALSE': 'If TOS is false, sets the bytecode counter to target and removes TOS',
        'POP_JUMP_IF_TRUE': 'If TOS is true, sets the bytecode counter to target and removes TOS',
        'LOAD_GLOBAL': 'Pushes the value associated with co_names[namei] onto the stack',
        'CONTINUE_LOOP': 'Restarts a loop from the block stack',
        'SETUP_LOOP': 'Pushes a block for a loop onto the block stack',
        'SETUP_EXCEPT': 'Pushes a block for an exception handler onto the block stack',
        'SETUP_FINALLY': 'Pushes a block for a "finally" handler onto the block stack',
        'LOAD_FAST': 'Pushes a local variable onto the stack',
        'STORE_FAST': 'Stores TOS as a local variable',
        'DELETE_FAST': 'Deletes a local variable',
        'RAISE_VARARGS': 'Raises an exception specified by the number of args',
        'CALL_FUNCTION': 'Calls a function with nargs positional arguments and nkwargs keyword arguments',
        'MAKE_FUNCTION': 'Pushes a new function object on the stack',
        'BUILD_SLICE': 'Builds a slice object from the topmost items on the stack',
        'LOAD_CLOSURE': 'Pushes a reference to a cell variable onto the stack',
        'LOAD_DEREF': 'Loads the cell contained in slot i of the cell and free variable storage',
        'STORE_DEREF': 'Stores TOS into the cell contained in slot i of the cell and free variable storage',
        'DELETE_DEREF': 'Deletes the cell contained in slot i of the cell and free variable storage',
        'CALL_FUNCTION_KW': 'Calls a function with nargs positional arguments, nkwargs keyword arguments, and a dictionary of keyword arguments',
        'CALL_FUNCTION_EX': 'Calls a function with combined nargs and nkwargs arguments',
        'SETUP_WITH': 'Sets up a with statement',
        'EXTENDED_ARG': 'Prefixes any opcode which has an argument too big to fit into the default two bytes',
        'LIST_APPEND': 'Calls list.append(TOS[-i], TOS) for each i in arg',
        'SET_ADD': 'Calls set.add(TOS[-i], TOS) for each i in arg',
        'MAP_ADD': 'Calls dict.setitem(TOS1[-i], TOS, TOS1) for each i in arg',
    }
